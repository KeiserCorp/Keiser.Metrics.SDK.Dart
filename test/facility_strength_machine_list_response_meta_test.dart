import 'package:test/test.dart';
import 'package:keiser_metrics_sdk/keiser_metrics_sdk.dart';

// tests for FacilityStrengthMachineListResponseMeta
void main() {
  final FacilityStrengthMachineListResponseMeta? instance = /* FacilityStrengthMachineListResponseMeta(...) */ null;
  // TODO add properties to the entity

  group(FacilityStrengthMachineListResponseMeta, () {
    // String model
    test('to test the property `model`', () async {
      // TODO
    });

    // String source_
    test('to test the property `source_`', () async {
      // TODO
    });

    // FacilityStrengthMachineSorting sort
    test('to test the property `sort`', () async {
      // TODO
    });

    // num facilityId
    test('to test the property `facilityId`', () async {
      // TODO
    });

    // bool ascending
    test('to test the property `ascending`', () async {
      // TODO
    });

    // num limit
    test('to test the property `limit`', () async {
      // TODO
    });

    // num offset
    test('to test the property `offset`', () async {
      // TODO
    });

    // num totalCount
    test('to test the property `totalCount`', () async {
      // TODO
    });

  });
}
