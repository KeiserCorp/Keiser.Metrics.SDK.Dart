import 'package:test/test.dart';
import 'package:keiser_metrics_sdk/keiser_metrics_sdk.dart';

// tests for FacilityUserRelationshipListResponseMeta
void main() {
  final FacilityUserRelationshipListResponseMeta? instance = /* FacilityUserRelationshipListResponseMeta(...) */ null;
  // TODO add properties to the entity

  group(FacilityUserRelationshipListResponseMeta, () {
    // bool member
    test('to test the property `member`', () async {
      // TODO
    });

    // bool employee
    test('to test the property `employee`', () async {
      // TODO
    });

    // String names
    test('to test the property `names`', () async {
      // TODO
    });

    // String memberIdentifier
    test('to test the property `memberIdentifier`', () async {
      // TODO
    });

    // EmployeeRole employeeRole
    test('to test the property `employeeRole`', () async {
      // TODO
    });

    // bool includeSession
    test('to test the property `includeSession`', () async {
      // TODO
    });

    // FacilityUserRelationshipSorting sort
    test('to test the property `sort`', () async {
      // TODO
    });

    // num facilityId
    test('to test the property `facilityId`', () async {
      // TODO
    });

    // bool ascending
    test('to test the property `ascending`', () async {
      // TODO
    });

    // num limit
    test('to test the property `limit`', () async {
      // TODO
    });

    // num offset
    test('to test the property `offset`', () async {
      // TODO
    });

    // num totalCount
    test('to test the property `totalCount`', () async {
      // TODO
    });

  });
}
